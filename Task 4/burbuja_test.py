...
class PruebasBurbuja(unittest.TestCase):
    ...
    def test_mejor_caso(self):
        print()
        best, mejor_caso = big_o.big_o(burbuja_optimizado, generar_mejor, min_n=10, max_n=1000,
                                   n_measures=100, n_repeats=3, verbose=False,
                                   classes=[cmpl.Linear, cmpl.Quadratic], return_raw_data=True)

        _graficar(mejor_caso, 'Mejor caso')
    ...
    def test_peor_caso(self):
        print()
        best, peor_caso = big_o.big_o(burbuja_optimizado, generar_peor, min_n=10, max_n=1000,
                                   n_measures=100, n_repeats=3, verbose=False,
                                   classes=[cmpl.Linear, cmpl.Quadratic], return_raw_data=True)

        _graficar(peor_caso, 'Peor caso')
    ...
    def test_caso_promedio(self):
        print()
        best, caso_promedio = big_o.big_o(burbuja_optimizado, generar_promedio, min_n=10, max_n=1000,
                                   n_measures=100, n_repeats=3, verbose=False,
                                   classes=[cmpl.Linear, cmpl.Quadratic], return_raw_data=True)

        _graficar(caso_promedio, 'Caso promedio')
